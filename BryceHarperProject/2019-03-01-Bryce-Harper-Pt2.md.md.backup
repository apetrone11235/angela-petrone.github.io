...
To evaluate Bryce Harper's value to the Phillies, I focused on his historic offensive statistics, focusing on his run production. I was much less concerned with his defensive ability because, as an outfielder, he is unlikely to change the score of a given game on the field. Of course, a truly terrible outfielder who misses routine fly balls and can't make a decent throw to the plate will cost their team some runs, but Harper is not known to be a liability in the outfield.
...
_As a disclaimer, I understand that 8 years of data are generally not considered enough to use in a model. In part, that is a point I am trying to make: locking in an unpredictable player for more than a decade is very risky. Also, I wanted more practice working with data and modeling, so I was undeterred by this prospect._

![]({{site.baseurl}}/img/bhRunsggplot.png)

The first thing I noticed about this graph was his inconsistency in run production. In 2014 when he was just 22, Harper only played 100 games due to a knee injury that required surgery. It seems like the surgery was not just successful but also gave him either a lot of motivation to perform in 2015 or hitting superpowers, because in 2015 he came back to lead the league in runs and HRs, lead the entire MLB in OBP and SLG, make the All Star team, win the NL [MVP award](https://en.wikipedia.org/wiki/Major_League_Baseball_Most_Valuable_Player_Award), and win the NL [Silver Slugger Award](https://en.wikipedia.org/wiki/Silver_Slugger_Award).

While these two seasons are definitely critical for understanding Harper as an offensive player, they make his stats pretty challenging to predict. This problem was amplified by the general lack of data. For example, when I took a look at the relationship between runs scored, OBP and SLG, I got this:

![]({{site.baseurl}}/img/bhHittingStatsgg.png)

Statistically, the results of the Moneyball model were unusable with no significant coefficients and massive error terms.
...
This was not an unexpected discovery and I decided to see if there was any statistically significant way to predict Harper's run production. From here, I tried many different combinations of the dependent variables and will leave you with what I found to be the most successful model I could find. _(Note that it was the most successful, not necessarily totally successful.)_

**Revised Model: OBP + SLG + G**
...
This result was better than my initial model, although the error terms were still very large. 

#### Analysis

The first thing I wanted to check for was if there was collinearity between variables.
...
Additionally, this histogram shows that the mean of the residuals is close to 0 but that the data might be skewed. I checked the summary and tested for skewness and found that the distribution was approximately symmetric:
...
Compared to other potential models I tested, these results were overall the best although not ideal.


#### Prediction

I wanted to see how many runs this model predicted and how it compared to other predictive resources out there. The equation we have is:
...
This felt pretty high but not totally unreasonable given Harper's past production when he plays nearly all of the games in a season. Baseball-Reference has [their projection](https://www.baseball-reference.com/players/h/harpebr03.shtml) at 90 runs. Based on Harper's projected PAs, it seems that Baseball-Reference is assuming he will play approximately 140 games which is about his career average. Similarly, [MLB Fantasy Rankings](https://www.mlb.com/news/fantasy-baseball-rankings-2019-player-preview-c295284374) has him producing 94 runs.
